...
public class MaintenanceAPI {
	...
	public Response getAllMaintenance() {
		...
		return Response.status(Status.OK).entity(maintenanceList).build();
	}
	...
	public Response createProduct(String productJson) {
		try {
			...
			if (!maintenance.createMaintenance()) {
				return Response.status(Status.INTERNAL_SERVER_ERROR.getStatusCode(), "Unable to create a maintenance.")
						.build();
			}
	...
	public Response updateMaintenance(String jsonData) {
		try {
			...
			var maintenance = new Maintenance(json.getInt("id"), new Date(json.getLong("date")),
					json.getInt("duration"), json.getString("instructions"), json.getString("report"),
					MaintenanceStatus.valueOf(json.getString("status")));
			if (!maintenance.updateMaintenance()) {
				return Response.status(Status.INTERNAL_SERVER_ERROR).entity(maintenance).build();
			}
			return Response.status(Status.OK).build();
		} catch (JSONException e) {
			e.printStackTrace();
